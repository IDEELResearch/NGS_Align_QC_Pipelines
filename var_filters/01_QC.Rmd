---
title: "Whole Genome Coverage Status"
...
alignsumfiles <- dir(path= "<alignmentpath from picardtools step in snQCer>", full.names = TRUE)
...
flagsumfiles <- dir(path="<alignmentpath from samtools flagstats step in snQCer>", full.names = TRUE)
...
calllocisumfiles <- dir(path="<alignmentpath from GATK callable loci step in snQCer>", full.names = TRUE)
...
# read in
#------------------------
bamcovs <- NFBtools::bedtoolsgenomecov2bamCov(gencovdir = "<coverage path from bedtools genomecov step in snQCer>")
